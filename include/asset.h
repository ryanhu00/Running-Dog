...
typedef enum { ASSET_IMAGE, ASSET_FONT, ASSET_BUTTON, ASSET_BACKGROUND} asset_type_t;
...
/**
 * Allocates memory for a background asset with the given parameters.
 * Note that `image_asset` should be owned by the caller. Thus,
 * `asset_destroy` will only free the memory allocated for the button.
 *
 * Asserts that `image_asset` is NULL or has type `ASSET_IMAGE`.
 *
 ...
 *
 * @param background the background to shift
 */
ssize_t asset_background_tick(asset_t *background);

/**
 * Returns x position of background
 *
